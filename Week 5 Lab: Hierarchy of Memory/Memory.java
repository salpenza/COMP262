...
public class Memory {
    ...
    // Used when replacing a value in a full cache.
    public String findOldestInCache() {
        String minKey = "";
    ...
    public void load(int sourceType, int address) {
        accessCacheTime();
        String key = (sourceType) + " " + (address / 1000);
        if (!(cache.containsKey(key))) {
            if (cache.size() > 11) {
                cache.remove(findOldestInCache());
            }
            key = (sourceType) + " " + (address / 1000);
            cache.put(key, time());
    ...
    // Makes any necessary changes to the contents of the cache and the time statistics
    public void copy(int sourceType, int sourceAddress, int destType, int destAddress) {
        load(sourceType, sourceAddress);
        readCacheTime();
        writeToType(destType);
    }

    // Reads in the instructions until the stop command is received after which it prints the statistics.
    ...
    public void fileParser(File file) throws FileNotFoundException {
        ...
        while(scan.hasNextLine()){
            ...
            if(line[0].equals("LOAD")){
                load(Integer.parseInt(line[1]), Integer.parseInt(line[2]));
            }else if(line[0].equals("COPY")){
                copy(Integer.parseInt(line[1]), Integer.parseInt(line[2]), Integer.parseInt(line[3]), Integer.parseInt(line[4]));
